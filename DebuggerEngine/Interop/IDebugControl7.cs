...
namespace DebuggerEngine.Interop {
	[ComImport, InterfaceType(ComInterfaceType.InterfaceIsIUnknown), Guid("b86fb3b1-80d4-475b-aea3-cf06539cf63a")]
	public unsafe interface IDebugControl7 : IDebugControl6 {
		...
		/* IDebugControl6 */
		[PreserveSig]
		new int GetExecutionStatusEx([Out] out DEBUG_STATUS Status);

		[PreserveSig]
		new int GetSynchronizationStatus(
		...
		int GetDebuggeeType2(
			[In] GET_DEBUGGEE_TYPE2_FLAGS flags,
			[Out] out DEBUG_CLASS Class,
			[Out] out DEBUG_CLASS_QUALIFIER Qualifier);
