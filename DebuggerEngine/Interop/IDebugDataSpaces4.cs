...
namespace DebuggerEngine.Interop
{
	...
	public unsafe interface IDebugDataSpaces4 : IDebugDataSpaces3
	{
		/* IDebugDataSpaces */
		...
		/* IDebugDataSpaces2 */
		[PreserveSig] new int VirtualToPhysical(
		    [In] UInt64 Virtual,
		    [Out] out UInt64 Physical);
		[PreserveSig] new int GetVirtualTranslationPhysicalOffsets(
		    [In] UInt64 Virtual,
		    [Out, MarshalAs(UnmanagedType.LPArray)] UInt64[] Offsets,
		    [In] UInt32 OffsetsSize,
		    [In] UInt32* Levels);
		[PreserveSig] new int ReadHandleData(
		    [In] UInt64 Handle,
		    [In] DEBUG_HANDLE_DATA_TYPE DataType,
		    [In] IntPtr Buffer,
		    [In] UInt32 BufferSize,
		    [In] UInt32* DataSize);
		[PreserveSig] new int FillVirtual(
		    [In] UInt64 Start,
		    [In] UInt32 Size,
		    [In] IntPtr Pattern,
		    [In] UInt32 PatternSize,
		    [In] UInt32* Filled);
		[PreserveSig] new int FillPhysical(
		    [In] UInt64 Start,
		    [In] UInt32 Size,
		    [In] IntPtr Pattern,
		    [In] UInt32 PatternSize,
		    [In] UInt32* Filled);
		[PreserveSig] new int QueryVirtual(
		    [In] UInt64 Offset,
			[In] IntPtr Info_Aligned_MEMORY_BASIC_INFORMATION64);
		    //[Out] out MEMORY_BASIC_INFORMATION64 Info);

		/* IDebugDataSpaces3 */
		[PreserveSig] new int ReadImageNtHeaders(
		    [In] UInt64 ImageBase,
		    [Out] out IMAGE_NT_HEADERS64 Headers);
		[PreserveSig] new int ReadTagged(
		    [In, MarshalAs(UnmanagedType.LPStruct)] Guid Tag,
		    [In] UInt32 Offset,
		    [In] IntPtr Buffer,
		    [In] UInt32 BufferSize,
		    [In] UInt32* TotalSize);
		[PreserveSig] new int StartEnumTagged(
		    [Out] out UInt64 Handle);
		[PreserveSig] new int GetNextTagged(
		    [In] UInt64 Handle,
		    [Out] out Guid Tag,
		    [Out] out UInt32 Size);
		[PreserveSig] new int EndEnumTagged(
		    [In] UInt64 Handle);

		/* IDebugDataSpaces4 */
		[PreserveSig] int GetOffsetInformation(
			...
			[In] UInt32 BufferSize,
			[In] UInt32* InfoSize);
		[PreserveSig] int GetNextDifferentlyValidOffsetVirtual(
			[In] UInt64 Offset,
			[Out] out UInt64 NextOffset);
		[PreserveSig] int GetValidRegionVirtual(
			[In] UInt64 Base,
			[In] UInt32 Size,
			[Out] out UInt64 ValidBase,
			[Out] out UInt32 ValidSize);
		[PreserveSig] int SearchVirtual2(
			[In] UInt64 Offset,
			[In] UInt64 Length,
			...
			[In] UInt32 PatternGranularity,
			[Out] out UInt64 MatchOffset);
		[PreserveSig] int ReadMultiByteStringVirtual(
			[In] UInt64 Offset,
			[In] UInt32 MaxBytes,
			[Out, MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer,
			[In] UInt32 BufferSize,
			[In] UInt32* StringBytes);
		[PreserveSig] int ReadMultiByteStringVirtualWide(
			[In] UInt64 Offset,
			[In] UInt32 MaxBytes,
			...
			[In] UInt32 BufferSize,
			[In] UInt32* StringBytes);
		[PreserveSig] int ReadUnicodeStringVirtual(
			[In] UInt64 Offset,
			[In] UInt32 MaxBytes,
			[In] CODE_PAGE CodePage,
			[Out, MarshalAs(UnmanagedType.LPStr)] StringBuilder Buffer,
			[In] UInt32 BufferSize,
			[In] UInt32* StringBytes);
		[PreserveSig] int ReadUnicodeStringVirtualWide(
			[In] UInt64 Offset,
			[In] UInt32 MaxBytes,
			[Out, MarshalAs(UnmanagedType.LPWStr)] StringBuilder Buffer,
			[In] UInt32 BufferSize,
			[In] UInt32* StringBytes);
		[PreserveSig] int ReadPhysical2(
			[In] UInt64 Offset,
			[In] DEBUG_PHYSICAL Flags,
			[In] IntPtr Buffer,
			[In] UInt32 BufferSize,
			[In] UInt32* BytesRead);
		[PreserveSig] int WritePhysical2(
			[In] UInt64 Offset,
			[In] DEBUG_PHYSICAL Flags,
			[In] IntPtr Buffer,
			[In] UInt32 BufferSize,
