...
namespace DebuggerEngine.Interop
{
	...
	public unsafe interface IDebugDataSpaces3 : IDebugDataSpaces2
	{
		/* IDebugDataSpaces */
		...
		/* IDebugDataSpaces3 */
		[PreserveSig] int ReadImageNtHeaders(
			[In] UInt64 ImageBase,
			[Out] out IMAGE_NT_HEADERS64 Headers);
		[PreserveSig] int ReadTagged(
			[In, MarshalAs(UnmanagedType.LPStruct)] Guid Tag,
			[In] UInt32 Offset,
			[In] IntPtr Buffer,
			[In] UInt32 BufferSize,
			[In] UInt32* TotalSize);
		[PreserveSig] int StartEnumTagged(
			[Out] out UInt64 Handle);
		[PreserveSig] int GetNextTagged(
			[In] UInt64 Handle,
			[Out] out Guid Tag,
			[Out] out UInt32 Size);
		[PreserveSig] int EndEnumTagged(
			[In] UInt64 Handle);
	}
}
